...
#endif
	if(!(pt->inner_page_tables[opt]->pt_dir[pde][pte] & ARCH_VM_PTE_PRESENT)) {
...
#if defined(__i386__)
	if(writeflag &&
		!(pt->inner_page_tables[opt]->pt_dir[pde][pte] & ARCH_VM_PTE_RW)) {
...
/* Allocate a page table and write its address into the page directory. */
	...
	/* Get storage for the page table. */
        if(!(opt->inner_page_tables[opt]->pt_dir[pde] = vm_allocpage(&pt_phys, VMP_PAGETABLE)))
		return ENOMEM;
...
int opt_new(opt_t *pt){
	...
	if (!pt->inner_page_tables && 
	    !(pt->inner_page_tables = vm_allocpages((phys_bytes *), VMP_PAGEDIR,  ARCH_PAGEDIR /  VM_PAGE_SIZE )))
		return ENOMEM;
}


	for ( i = 0; i < ARCH_VM_DIR_ENTRIES; i++){
		pt->inner_page_tables[i] = 0;
