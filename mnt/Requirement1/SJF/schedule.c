...
{
	register struct schedproc* rmp, test;
	int rv, proc_nr_n;
	...
	if (rmp->priority > 4)
	{
		...
		for (int i = 0; i < NR_PROCS; i++)
		{
			...
			if (rmp->flags = IN_USE)
			{
				if (rmp->time_slice < minTime || i == 0)
				{
					minTime = rmp->time_slice;
					test = rmp;
				}
			}
		}
	...
	else // then it is a kernel process
	{
	...
	// deleted the feedback enha tnzl lel queue ely b3do
...
int do_nice(message* m_ptr)
{
	...
	int rv;
	int proc_nr_n;
	unsigned new_q, old_q, old_max_q, time_slice;// WE ADDED the time slice
	...
	if ((rv = schedule_process_local(rmp)) != OK) {
		...
		rmp->time_slice = time_slice;////////////////////////////
	}

	return rv;
}
/*===========================================================================*
 *				schedule_process	///////SJF	     *
 *===========================================================================*/
static int schedule_process(struct schedproc* rmp, unsigned flags)
{
	int err;
	int new_priority, new_quantum, new_cpu;
	...
	if ((err = sys_schedule(rmp->endpoint, new_priority,
