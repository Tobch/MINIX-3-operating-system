...
/* Allocate a bit from a bit map and return its bit number. */
  ...
  do {
	...
	for (wptr = &b_bitmap(bp)[word]; wptr < wlim; wptr++) {
		...
		for (i = 0; count < USER_DEFINED_SIZE && i<16;++i){
                  if((k & (1 << i)) == 0){
                          count++;
                     }
                 }
                     if (count!= USER_DEFINED_SIZE) continue;
		...
		/* Allocate and return bit number. */
                 for(int j=i;j>i-USER_DEFINED_SIZE;J--){
...
/* Return a zone or inode by turning off its bitmap bit. */
  ...
  for(int j=bit; j>bit-USER_DEFINED_SIZE;j--){
    mask = 1 << bit;
  }
...
u32_t get_usedBlocks(struct super_block *sp)
{
   if(!visited) {

       usedBlocks = sp->s_zones - count_free_bits(sp, ZMAP);
       visited = 1;
     }
